...
delta_seq    <- seq(from = 0, to = 0.1, len = 10)
N_seq        <- c(10,100)
## #########################################################
...
for(DGP in DGP_seq) {
  ...
  for(N in N_seq) {
    for(delta in delta_seq) {# DGP <- DGP_seq[2]; N <- N_seq[1]; delta <- delta_seq[1]
      ...
      for(i in 1:reps){# 
        ...
        # plot(b); lines(x=grid, y=mu, col="blue")
        ## ==============================================================================================
      ...
      crossings_df <- dplyr::tibble(
        !!Band_type[1] :=  c(crossings_loc[,,1]),
        !!Band_type[2] :=  c(crossings_loc[,,2]),
        !!Band_type[3] :=  c(crossings_loc[,,3]),
        !!Band_type[4] :=  c(crossings_loc[,,4]),
        !!Band_type[5] :=  c(crossings_loc[,,5]),
        "IWT"           =  c(crossings_loc_IWT)) %>% 
        tidyr::gather(band, cr_loc, factor_key=TRUE) %>% 
        dplyr::mutate(intervals = as.factor(findInterval(x=cr_loc, vec=seq(0,1,len=5), rightmost.closed=T)))
  ...
  save(sim_df, file = paste0("Simulation_Results/", DGP))
}# DGP-loop
