...
DGP_seq      <- c("DGP1_shift","DGP1_scale","DGP1_local",
                  ...
                  "DGP3_shift","DGP3_scale","DGP3_local_2")[9]
##
delta_Nsmall  <- 0.8#c(0, seq(from = 0.05, to = 0.45, len = 5))
delta_Nlarge  <- 0.8#c(0, seq(from = 0.02, to = 0.1,  len = 5))
##
N_seq         <- c(15, 100)[2]
## #########################################################
...
for(DGP in DGP_seq) {
  ...
  for(N in N_seq) {
    ...
    ##
    for(delta in delta_seq) {# DGP <- "DGP3_local_2"; N <- 100; delta <- 0.08
      ## 
      ...
      if(grepl("local", DGP)) { mu0 <- meanf_rect( grid, 0);  mu <- meanf_rect( grid, delta, rect = c(2/3+c(0,0.125))) }
      names(mu)  <- grid
      names(mu0) <- grid
      ##
