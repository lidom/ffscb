...
for(DGP in DGP_seq) {
  ...
  for(fragm_len in fragm_len_v) {
    ...
    for(delta in delta_seq) {# DGP <- "DGP1_shift"; N <- N; fragm_len <- fragm_len_v[1]; delta <- 0
      ...
      ##
      save(sim_df, file = paste0(my_path, "Simulation_Results/", DGP, "_N=", N, "_alpha=", alpha.level, "_t0=", t0, "_fragm=",fragm_len,"_Delta=", delta,".RData"))
    }# delta-loop
  }# fragm-loop
}# DGP-loop
...
for(i in 1:3){
  for(dgp in c(paste0("DGP",i,"_scale"), paste0("DGP",i,"_local"))) {
    for(fragm_len in fragm_len_v) {
      ##
      load(file = paste0(my_path, "Simulation_Results/DGP",i,"_shift", "_N=", N, "_alpha=", alpha.level, "_t0=", t0, "_fragm=", fragm_len, "_Delta=0.RData"))
      sim_df <- sim_df %>% mutate(DGP = dgp) # Replace name of DGP
      save(sim_df, file = paste0(my_path, "Simulation_Results/",  dgp, "_N=", N, "_alpha=", alpha.level, "_t0=", t0, "_fragm=", fragm_len, "_Delta=0.RData"))
      rm(sim_df)
    }
  }
}
