...
make.band.FFSCB.z <- function(tau, t0, conf.level, n_int){
  ...
  if(n_int == 1){# Case n_int=1 == constant band == Kac-Rice Band
    ...
    myfun1     <- function(c1){pnorm(q=c1, lower.tail=F)+exp(-c1^2/2)*tau01/(2*pi)-(alpha.level/2)}
  ...
  find_u <- function(alpha.aux){# alpha.aux = 0.006
    ...
    if(const_int > 1){
      for(j in (const_int-1):1){
        myfunj <- function(cj){
          ...
          fn2    <- function(t,cj){sum(c(c_v_sum,cj))/sqrt(2*pi) * exp(-ufun_j(t,cj)^2/2) * pnorm( sum(c(c_v_sum, cj))/tau_f(t))}
    ...
    if(const_int < n_int){
      for(j in (const_int+1):n_int){# j <- (const_int+1)
        myfunj <- function(cj){
          ...
          ##
          fn1    <- function(t,cj){(tau_f(t)/(2*pi)) * exp(-ufun_j(t,cj)^2/2) * exp(-sum(c(c_v_sum,cj))^2/(2*tau_f(t)^2))}
          fn3    <- function(t,cj){sum(c(c_v_sum,cj))/sqrt(2*pi) * exp(-ufun_j(t,cj)^2/2) * pnorm(-sum(c(c_v_sum, cj))/tau_f(t))}
          intgr1 <- sum(fn1(t=tt[knots[j] < tt & tt <= knots[j+1]], cj=cj)) * diff(tt)[1]
    ...
    fn2_star <- function(t){up_star_f(t)/sqrt(2*pi) * exp(-u_star_f(t)^2/2) * pnorm( up_star_f(t)/tau_f(t))}
    fn3_star <- function(t){up_star_f(t)/sqrt(2*pi) * exp(-u_star_f(t)^2/2) * pnorm(-up_star_f(t)/tau_f(t))}
    ##
    ...
    ##
    optim_target <- c( (pnorm(q=u_star_f(t0), lower.tail=F) + intgr1_star + intgr2_star - intgr3_star) - alpha.level/2)^2
  ...
  ##
  opt_res <- optimize(f = function(x){find_u(alpha.aux = x)$optim_target}, interval = c(0,alpha.level), tol=tol)$minimum
  ##
  ...
  find_u <- function(alpha.aux){
        ...
        myfunj <- function(cj){
          ...
          fn2 <- function(t,cj){
            (sum(c(c_v_sum,cj))/(2*pi*tau_f(t))) * (1+ufun_j(t,cj)^2/nu)^(-nu/2 -1)  *
              ...
              pt(q = (sum(c(c_v_sum,cj)) / afun_j(t,cj) ), df=nup) 
          }
    ...
    fn2_star <- function(t){
      (up_star_f(t)/(2*pi*tau_f(t))) * (1+u_star_f(t)^2/nu)^(-nu/2 -1)  *
        ...
        pt(q = (up_star_f(t) / a_star_f(t) ), df=nup) 
    }
    fn3_star <- function(t){
      (up_star_f(t)/(2*pi*tau_f(t))) * (1+u_star_f(t)^2/nu)^(-nu/2 -1)  *
        (gamma(nup/2) * sqrt(nup*pi) * a_star_f(t) / gamma((nup+1)/2) ) *
    ...
    if(t0<1){intgr3_star <- sum(fn3_star(t=tt[t0 <  tt])) * diff(tt)[1]}else{intgr3_star <- 0}
    ##
    optim_target <- c( (pt(q=u_star_f(t0), lower.tail=F, df = nu) + intgr1_star + intgr2_star - intgr3_star) - alpha.level/2)^2
    band.eval    <- u_star_f(t=seq(0,1,len=length(tau)))
    ##
    return(list("optim_target" = optim_target,
                "band.eval"    = band.eval))
